...
namespace CCC_27_Rathaus.Levels
{
	...
	{
		public string ID;
		public List<Element4In> eIn;
		public List<Element4Out> eOut;
		public long Time;
		...
		internal bool FindAllInputElements(List<Transaction4> validTransactions)
		{
			var weAreOk = true;
	...
	public class Element4In
	{
		public string InID;
		public string Owner;
		public double Amount;
	}
	public class Element4Out
	{
		public string Owner;
		public double Amount;
	...
	public class Level4 // : BaseStuff.ICccLevel<string[]>
	{
		public string[] CalculateResult(string inputFileName)
		{
			Dictionary<long, Transaction4> transactions = null;
			ParseInput(inputFileName, out transactions);

			var sortedXAct = transactions.OrderBy(t => t.Key);
			List<Transaction4> validTransactions = new List<Transaction4>();
		...
		void ParseInput(string inputFile, out Dictionary<long, Transaction4> Transactions)
		{
			var lines = BaseStuff.CccTest.ReadInputFile(inputFile);

			var numXAct = int.Parse(lines[0][0]);
			Transactions = new Dictionary<long, Transaction4>(numXAct);

			for (int t = 1; t < numXAct + 1; t++) {
				int Column = 0;
				var xact = new Transaction4() { ID = lines[t][Column++] };
				var numIn = int.Parse(lines[t][Column++]);

				xact.eIn = new List<Element4In>();
				for (int inIdx = 0; inIdx < numIn; inIdx++) {
					var inEl = new Element4In();
				...
				xact.eOut = new List<Element4Out>();
				for (int outIdx = 0; outIdx < numOut; outIdx++) {
					var outEl = new Element4Out();
